...
const addMembers =TryCatch(async (req, res, next) => {
   ...
   //allMembersPromise can allnewmemberpromise 
    ...
    //  chat.members.push(...allNewMembers.map((i)=>i._id)); //for duplicate members
    ...
    await chat.save();
      ...
      emitEvent
      (req, ALERT, chat.members, `${allUsersName} has been  added to  group`);
...
const removeMember =TryCatch(async (req, res, next) => {
   ...
   if(chat.creator.toString() !== req.user.toString()) 
       return next(new ErrorHandler("You are not allowed to add on this group", 403));
...
const leaveGroup =TryCatch(async (req, res, next) => {
    ...
    if(remainingMembers.length < 3) return next(new ErrorHandler("Group must have 3 members", 400));
 ...
 return res.status(200).json({
  ...
  const sendAttachments = TryCatch(async(req, res, next) => {
    ...
    if(files.length > 5)
          return next(new ErrorHandler("Only 5 attachments allowed", 400));
